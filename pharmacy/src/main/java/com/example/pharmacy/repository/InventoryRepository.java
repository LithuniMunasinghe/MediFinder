...
public interface InventoryRepository extends JpaRepository<Inventory, Long> {
    ...
    @Query("SELECT new com.example.pharmacy.dto.InventoryDTO(" +
            "i.id, m.name, m.description, i.quantity, i.price) " +
            "FROM Inventory i JOIN i.medicine m " +
            "WHERE i.pharmacy.id = :pharmacyId")
    List<InventoryDTO> findByPharmacyId(@Param("pharmacyId") Long pharmacyId);
